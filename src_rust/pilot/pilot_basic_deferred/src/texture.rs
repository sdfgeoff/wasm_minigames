...
pub fn load_texture(gl: &GL, image_bytes: &[u8]) -> Result<Option<WebGlTexture>, TextureError> {
    ...
    if texture.is_none() {
        Err(TextureError::AllocateTextureError)
    } else {
        Ok(texture)
    }
}
...
pub fn set_up_image(gl: &GL, img_element: &HtmlImageElement, texture: &Option<WebGlTexture>) {
    gl.bind_texture(GL::TEXTURE_2D, texture.as_ref());
