...
void main() {
    ...
    float depth = normal_depth.a; // Distance in meters from the camera

    float fog_depth = depth / 50.0;
    float fog = 1.0 / (fog_depth * fog_depth + 1.0);//pow(depth, 0.5) * 1.5;
    
    vec3 lighting = sample_background(normal_depth.xyz, 3.0);
    
    vec3 out_col = lighting * albedo.rgb;
    
    out_col = mix(vec3(1.0, 1.0, 1.0), out_col, fog);
